##############################################################################
#Adapted from a tutorial by @BrockTibert
library(RCurl)
library(XML)
...
#2017-18 URL format:
#https://www.cbssports.com/fantasy/basketball/stats/sortable/points/C/standard/projections/2017/ytd
#https://www.cbssports.com/fantasy/basketball/stats/sortable/points/C/standard/projections/2017/ytd?&print_rows=9999
...
cbs_data <- function(pos, sport, stat_type){
  ...
  pred_URL_tail <- "/projections/2017/ytd?&print_rows=9999"
...
#Ignoring TO and 3PTM, what is the average z score
...
data.working$Zavg <- apply(data.working[,c(27:28,29,30:32,33,34:35)], 1, mean)
data.working[order(data.working$Zavg, decreasing=T),][1:20,c(1,37)]

data.working$Zavg_noTPG <- apply(data.working[,c(27:28,29,30:32,34:35)], 1, mean)
data.working[order(data.working$Zavg_noTPG, decreasing=T),][1:20,c(1,38)]

data.working$Zavg_no3PTM <- apply(data.working[,c(27:28,30:32,33,34:35)], 1, mean)
num_rows = 20
data.working[order(data.working$Zavg_no3PTM, decreasing=T),][1:num_rows,c(1,17,39)] %>% 
  group_by(Pos) %>% 
  summarise(count = n()) %>% 
  mutate(ratio = count/num_rows)
  
data.working %>% 
  arrange(Zavg_no3PTM)

ggplot(data.working[1:50,], aes(as.vector(data.working$ZvorTPG[1:50,]))) + geom_histogram()
ggplot(data.working[1:100,], aes(as.vector(data.working$ZvorFTP[1:100,]))) + geom_histogram()
  

write.csv(data.working, file="fantasy_ranking_2017.csv")
